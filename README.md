...
cd /d "C:\Users\Egor\Desktop\\app\LD"
...
2.  **Убедитесь, что папка для дистрибутива существует** (`C:\Users\Egor\Desktop\app\LD`) и что вы **скопировали в нее `LD7.exe`** из папки сборки.
...
*   **Хранение данных** - состояние приложения (все группы, участки, датчики и их записи) сериализуется в JSON-файл `data.json`. Это обеспечило простоту разработки, но может увеличить время отклика приложения из-за того, что на любую операцию по изменению этого файла тратится время пропорциональное его размеру. То есть чем больше файл, тем больше время отклика. На данном этапе это нормально. Далее потребуется либо куда то сбрасывать накопившуюся информацию, в другой файл, либо подкручивать БД.
