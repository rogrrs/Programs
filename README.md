# Руководство разработчика по проекту LD7

## 1. Обзор проекта

**LD7** — это настольное приложение на базе Qt 6, предназначенное для мониторинга, управления и анализа данных, поступающих от сети удаленных датчиков.
...
*   **Загрузка данных:** Поддерживается два режима загрузки данных:
    ...
    2.  **По UART** - прямая загрузка данных с устройства через последовательный порт.
...
| **Версия Qt** | Qt 6.x | Qt 6.5.3 или новее |
| **Компилятор** | MinGW 64-bit | MinGW 11.2.0 (поставляется с официальным инсталлятором Qt) |
| **Система сборки**| CMake | Версия 3.16 или новее (интегрирована в Qt Creator) |

### 2.1. Установка Qt и компонентов

1.  Скачайте **Qt Online Installer** с официального сайта Qt.
2.  При установке выберите версию Qt (например, **Qt 6.5.3**).
3.  В разделе компонентов убедитесь, что выбраны следующие модули:
    *   [x] **MinGW** (например, `MinGW 11.2.0 64-bit`)
    *   [x] **CMake**
    *   [x] **Qt 6 Core**
    *   [x] **Qt GUI**
    *   [x] **Qt Widgets**
...
2.  Убедитесь, что выбран **Kit**, который соответствует установленным вами компонентам (например, **Desktop Qt 6.5.3 MinGW 64-bit**).
3.  Нажмите кнопку **Configure Project**.

### Шаг 3 - Сборка и запуск
1.  На левой панели выберите режим сборки **Debug** или **Release**.
2.  Нажмите на иконку **молотка** (Build) на левой панели или выберите **Build → Build Project "LD7"**.
3.  После успешной сборки нажмите на иконку **зеленого треугольника** (Run) или нажмите **Ctrl+R**.

Приложение должно запуститься. В каталоге сборки (например, `build-LD7-Desktop_Qt_6_5_3_MinGW_64_bit-Debug`) будет создан исполняемый файл `LD7.exe`.
...
*   **Проблема** - проект не собирается, CMake жалуется на отсутствующие компоненты Qt.
    *   **Решение** - убедитесь, что все модули, перечисленные в пункте **2.1**, установлены для вашей версии Qt. Запустите **Qt Maintenance Tool** и доустановите недостающие компоненты.
